...
static void processVertex(DynamicArray *vertices, char *vertexData[3],
...
static DynamicArray *loadOBJ(const char *filename) {
...
Mesh *createMesh(const char *filename, bool instanced) {
  ...
  glBufferData(GL_ARRAY_BUFFER, sizeof(float) * STRIDE * mesh->numVertices,
